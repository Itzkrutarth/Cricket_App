...
const HomeContent = () => {
	...
	const pendingTaskCount = useSelector(
		(state: RootState) => state.task.tasks.filter((t) => !t.completed).length
	)
		...
		>
			...
			<View style={styles.header}>
				...
				<TouchableOpacity style={styles.notificationButton}>
					...
					{joinRequests.length + pendingTasks.length > 0 && (
						<View style={styles.notificationBadge}>
							<Text style={styles.badgeText}>
								{joinRequests.length + pendingTasks.length}
							</Text>
						</View>
					)}
				</TouchableOpacity>
