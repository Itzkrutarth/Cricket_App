...
export default function CoachesScreen() {
		...
		try {
			...
			try {
				...
				const reqRes = await fetch(
					`https://becomebetter-api.azurewebsites.net/api/GetJoinRequestsByStudentId?studentId=${studentId}`
				)
				...
				if (!Array.isArray(joinRequests)) {
					console.warn(
						"⚠️ Join request response is not an array. Defaulting to []"
					)
					joinRequests = []
				}
			...
			const formatted: Coach[] = coachesData.map((user: any) => ({
				...
				requestStatus: pendingCoachEmails.includes(
	...
	const handleAssignCoach = async (coach: Coach) => {
		...
		try {
			...
			const res = await fetch(
				`https://becomebetter-api.azurewebsites.net/api/AddCoachToStudent?`,
				{
					method: "PUT",
	...
	const handleSendJoinRequest = async (coach: Coach) => {
		...
		try {
			...
			const SEND_JOIN_REQUEST_KEY =
				process.env.EXPO_PUBLIC_SEND_JOIN_REQUEST_KEY
			const res = await fetch(
				`https://becomebetter-api.azurewebsites.net/api/SendJoinRequest?`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
						...
						<View key={coach.id} style={styles.card}>
							...
							{viewMode === "all" ? (
								coach.isAssigned ? (
									...
									<Text style={{ color: "#f39c12", fontWeight: "600" }}>
										⏳ Pending
									</Text>
								) : (
									<TouchableOpacity
